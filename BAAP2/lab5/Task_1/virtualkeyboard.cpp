...
VirtualKeyboard::VirtualKeyboard(QWidget* parent)
    : QWidget(parent), ui(new Ui::VirtualKeyboard) {
    _scene = new QGraphicsScene(0, 0, MAX_H_KEY_AMNT * (KEY_SIZE + KEY_SPOT),
                                MAX_V_KEY_AMNT * (KEY_SIZE + KEY_SPOT));
...
void VirtualKeyboard::initKeys() {
    ...
    std::array<QGraphicsRectItem*, 11> keys = {
        _tab = new QGraphicsRectItem(KEY_SPOT_HALF, ROW_2_Y, KEY_SIZE_1_5,
                                     KEY_SIZE),
        _caps =
            new QGraphicsRectItem(KEY_SPOT_HALF, ROW_3_Y, KEY_SIZE_2, KEY_SIZE),
        _lShift = new QGraphicsRectItem(KEY_SPOT_HALF, ROW_4_Y, KEY_SIZE_2_5,
                                        KEY_SIZE),
        _lCtrl = new QGraphicsRectItem(KEY_SPOT_HALF, ROW_5_Y, KEY_SIZE_2_5,
                                       KEY_SIZE),
        _lAlt = new QGraphicsRectItem(KEY_SPOT_1_5 + KEY_SIZE_2_5, ROW_5_Y,
                                      KEY_SIZE_2, KEY_SIZE),
        _space = new QGraphicsRectItem(KEY_SPOT_2_5 + KEY_SIZE_4_5, ROW_5_Y,
                                       KEY_SIZE_7, KEY_SIZE),
        _rAlt = new QGraphicsRectItem(KEY_SPOT_3_5 + KEY_SIZE_11_5, ROW_5_Y,
                                      KEY_SIZE_2, KEY_SIZE),
        _rCtrl = new QGraphicsRectItem(KEY_SPOT_4_5 + KEY_SIZE_13_5, ROW_5_Y,
                                       KEY_SIZE_2_5, KEY_SIZE),
        _back = new QGraphicsRectItem(
            KEY_SPOT_HALF + TOP_ROW_KEYS * (KEY_SIZE + KEY_SPOT), ROW_1_Y,
            KEY_SIZE_1_5, KEY_SIZE),
        _enter = new QGraphicsRectItem(
            KEY_SPOT_1_5 + KEY_SIZE_2 + MID_ROW_KEYS * (KEY_SIZE + KEY_SPOT),
            ROW_3_Y, KEY_SIZE_1_5 + KEY_SPOT, KEY_SIZE),
        _rShift = new QGraphicsRectItem(
            KEY_SPOT_1_5 + KEY_SIZE_2_5 + BOT_ROW_KEYS * (KEY_SIZE + KEY_SPOT),
            ROW_4_Y, KEY_SIZE_2_5 - KEY_SPOT, KEY_SIZE)};
    ...
    for (int i = 0; i < TOP_ROW_KEYS; ++i) {
        _highestRowBtn.at(i) =
            new QGraphicsRectItem(KEY_SPOT_HALF + i * (KEY_SIZE + KEY_SPOT),
                                  ROW_1_Y, KEY_SIZE, KEY_SIZE);
        _scene->addItem(_highestRowBtn.at(i));
    }


    for (int i = 0; i < TOP_ROW_KEYS; ++i) {
        _topRowBtn.at(i) = new QGraphicsRectItem(
            KEY_SPOT_1_5 + KEY_SIZE_1_5 + i * (KEY_SIZE + KEY_SPOT), ROW_2_Y,
            KEY_SIZE, KEY_SIZE);
    ...
    for (int i = 0; i < MID_ROW_KEYS; ++i) {
        _midRowBtn.at(i) = new QGraphicsRectItem(
            KEY_SPOT_1_5 + KEY_SIZE_2 + i * (KEY_SIZE + KEY_SPOT), ROW_3_Y,
            KEY_SIZE, KEY_SIZE);
    ...
    for (int i = 0; i < BOT_ROW_KEYS; ++i) {
        _botRowBtn.at(i) = new QGraphicsRectItem(
            KEY_SPOT_1_5 + KEY_SIZE_2_5 + i * (KEY_SIZE + KEY_SPOT), ROW_4_Y,
            KEY_SIZE, KEY_SIZE);
...
void VirtualKeyboard::initLetters() {
    ...
    for (int i = 0; i < TOP_ROW_KEYS; ++i) {
        addTextToButton(_highestRowBtn.at(i),
                        _keyboardLayout.at(_selectedLang).at(i));
        addTextToButton(_topRowBtn.at(i),
                        _keyboardLayout.at(_selectedLang).at(TOP_ROW_KEYS + i));
        if (i >= MID_ROW_KEYS) {
            continue;
        }
        addTextToButton(
            _midRowBtn.at(i),
            _keyboardLayout.at(_selectedLang).at(TOP_ROW_KEYS * 2 + i));
        if (i >= BOT_ROW_KEYS) {
            continue;
        }
        addTextToButton(_botRowBtn.at(i),
                        _keyboardLayout.at(_selectedLang)
                            .at(TOP_ROW_KEYS * 2 + MID_ROW_KEYS + i));
    }
}
...
void VirtualKeyboard::addTextToButton(QGraphicsRectItem* button,
                                      const QString& text) {
    QGraphicsTextItem* textItem =
        _scene->addText(text, QFont("Arial", KEY_SPOT, 10));
